...
class StopsTableViewController: UITableViewController {
    ...
    func observeStops(){
        ...
        let stopsRef = FIRDatabase.database().reference().child("citizen").child(uid!).child("stops")
        print("found stops")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Get officer name
        FIRDatabase.database().reference().child("officer").child("14567").child(arrayStopData[indexPath.row].officerID!).child("profile").observeSingleEvent(of: .value, with: { (snapshot) in
            
            if let dictionary = snapshot.value as? [String: AnyObject]{
                let name = dictionary["last_name"] as? String
                cell.driverNameLabel.text = "Officer " + name!
            }
        })
